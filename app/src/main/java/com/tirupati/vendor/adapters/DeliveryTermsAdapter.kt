...
class DeliveryTermsAdapter(val context: Context, var dataSource: List<UOMData>?) : BaseAdapter() {
...
//        label.setTypeface(typeface)
        label.setText(dataSource?.get(position)?.UOMCODE)
        return label
    ...
    fun getCustomView(position: Int, convertView: View?, parent: ViewGroup?): View? {
        ...
        textView.setText(dataSource?.get(position)?.UOMCODE)
        return textView
    }
